...
class ContentDuplicateChecker:
    def __init__(self, base_url, output_path: str, max_depth=2):
    ...
    def extract_content(self, soup):
        ...
        logging.debug(
            f"Extracted content: {content[:500]}..."
        ...
        for i in range(len(urls)):
            for j in range(i + 1, len(urls)):
                if csim[i, j] > 0.7:  # Threshold for considering as duplicate
                    ...
                    logging.info(
                        f"Found similar content between {urls[i]} and {urls[j]} with similarity {csim[i, j]:.2f}"
                    )
    ...
    def generate_report(self):
        ...
        df = pd.DataFrame(
            self.similarity_scores, columns=["URL_1", "URL_2", "Similarity"]
        )
